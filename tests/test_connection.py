...
async def test__placeholder_swap_mixed():
    """Ensure postgres double-colon casting doesn't
    conflict with placeholders
    """
    query = 'SELECT id, :color, id::text as text_id FROM meows WHERE id = :id'
    keyword_params = {'color': 'red', 'id': 3391}
    query_string, params = connection._placeholder_swap(
        query, keyword_params)
    assert query_string == 'SELECT id, $1, id::text as text_id FROM ' \
                           'meows WHERE id = $2'
    assert params == ['red', 3391]


async def test_compile_query(monkeypatch):
    def mock(*args):
        return 'bob', 'sally'
    monkeypatch.setattr('asyncpgsa.connection._placeholder_swap', mock)
    ids = list(range(1, 40))
    query = file_table.update() \
        .values(id=None) \
        .where(file_table.c.id.in_(ids))
